...
export function ChatInput({
    ...
    return (
        <div className={cn("relative flex flex-col gap-2 transition-all duration-300", className)}>
            ...
            {selectedFile && (
                <div className="absolute -top-12 left-0 flex items-center gap-2 px-3 py-1.5 glass-panel rounded-full w-fit text-xs animate-in slide-in-from-bottom-2">
            ...
            <div className="relative flex items-end gap-2 bg-background/60 backdrop-blur-xl border border-white/10 shadow-2xl rounded-3xl p-2 transition-all focus-within:ring-1 focus-within:ring-primary/50 focus-within:bg-background/80">
                <input
                    type="file"
                    className="hidden"
                    ref={fileInputRef}
                ...
                <Button
                    variant="ghost"
                    size="icon"
                    className="h-10 w-10 shrink-0 text-muted-foreground hover:text-primary rounded-full hover:bg-white/5 transition-colors"
